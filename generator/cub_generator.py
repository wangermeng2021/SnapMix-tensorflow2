...
class CubGenerator(tf.keras.utils.Sequence):
    def __init__(self, root_dir, mode='train', batch_size=32, augment='baseline'):
        img_paths = pd.read_csv(os.path.join(root_dir, "images.txt"), sep=" ", header=None, names=['idx', 'path'])
        img_labels = pd.read_csv(os.path.join(root_dir, "image_class_labels.txt"), sep=" ", header=None,  names=['idx', 'label'])
        train_test_split = pd.read_csv(os.path.join(root_dir, "train_test_split.txt"), sep=" ", header=None,  names=['idx', 'train_flag'])
        bounding_box = pd.read_csv(os.path.join(root_dir, "bounding_boxes.txt"), sep=" ", header=None,  names=['idx', 'x', 'y', 'w', 'h'])
        ...
        full_path_list = root_dir+"/images/"+imgs['path'].values
